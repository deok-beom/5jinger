...
public enum ErrorCode {
    ...
    IMPROPER_DEMOTION(BAD_REQUEST, "회원이 강등될 수 없는 등급이거나 이미 일반 고객 등급입니다."),
    INVALID_CUSTOMER_REQUEST(BAD_REQUEST, "자신이 등록한 상품에 요청할 수 없습니다."),
    NOT_CUSTOMERS(BAD_REQUEST, "CUSTOMER 등급의 고객만 등업을 신청할 수 있습니다."),
    INVALID_CREATE_ITEM(BAD_REQUEST, "제목, 내용, 가격이 올바르지 않습니다."),
    PASSWORD_NOT_MATCH(BAD_REQUEST, "비밀번호가 일치하지 않습니다."),
    ADMIN_PASSWORD_NOT_FOUND(BAD_REQUEST, "관리자 암호가 일치하지 않아 등록이 불가합니다."),
    REQUEST_IS_EXIST(BAD_REQUEST,"요청이 이미 존재합니다."),
    REQUEST_IS_REJECTED(BAD_REQUEST,"이미 거절한 요청입니다."),
    REQUEST_IS_EXPIRED(BAD_REQUEST,"요청이 이미 종결된 상태입니다."),
    ITEM_IS_EXPIRED(BAD_REQUEST,"상품이 수정/삭제 할 수 없는 상태입니다."),
    ITEM_IS_SOLD_OUT(BAD_REQUEST, "품절된 상품입니다."),
    ITEM_IS_SUSPENDED(BAD_REQUEST, "판매 중지된 상품입니다."),
    ...
    DUPLICATE_USERNAME(CONFLICT, "중복된 유저 이름 입니다."),
